...
We assume the user has installed a newer version of R, such as R >= 3.4.0, and knows how to install R packages. Besides that, the user has to install Julia by himself. The latest stable version is Julia 0.6.0 and can be downloaded from the Julia home page [julialang.org](https://julialang.org/) for the major operating systems Windows, macOS, and Linux.

Some Julia packages will be needed. Though the `julia_setup` routine that we will be mentioning soon tries to install some necessary packages, it is preferable to download and install these packages directly in Julia and before Julia is first called from R.
...
The command `Pkg.add()` will download and install Julia packages (in the `.julia` directory) once, and `using <pkg>` will load it into the active session. For instance, the *RCall* will enable Julia to call R routines or get access to R objects. It is necessary, e.g., when a function is user defined in R and shall be applied by a Julia routine, say for integration, optimization or in a differential equation.
...
# JuliaCall | Version 0.9.3.9000 (2017-10-11) | MIT + file LICENSE

library(JuliaCall)
julia_path = "/Applications/Julia-0.6.app/Contents/Resources/julia/bin"
julia_setup(julia_path)   # or julia_setup()
...
The setup reads the 'PATH' variable to find Julia. If the Julia `bin` directory is not present in the path, the path can be given as a parameter to the setup command. Another possibility is to set an option like (on macOS):
...
or use `julia_eval` on a complete Julia expression:
...
Please note that in *JuliaCall* all `Julia_...()` commands are also available as, for example, `jl$...()`, but this is only true if you have setup Julia with `jl = julia_setup()` -- or another name of your choice!
...
We have already seen some simple command for interacting between R and Julia. `julia_eval()` evaluates a string containing a correct Julia expression and returns the result to R. `julia_call()` accepts a Julia function name as string plus variables and returns the result of applying the function in Julia.

On the other hand, if there is an object like a vector or matrix in R, how can we make use of it in Julia. Enters the Julia *RCall* package that allows calling R from Julia. R objects are accessed through the handy notation `R"..."`.
...
The call `julia_eval('det(R"A")')` does not work because the type of `R"A"` in Julia is not a matrix, but an `RCall` object. Instead we need to convert this object into a corresponding Julia type. `julia_assign()` does this for us.
...
Let us try to define a function in R and integrate that function with a Julia integration routine. As a function we will take the Runge function.
...
To make this callable in Julia we have to tell it where to find the R function. This is done by *RCall* in Julia with the notation `R"fRunge"`, so applying it to a vector like this:

```r
julia_call('R"fRunge"', c(-1, -0-5, 0, 0.5, 1))
## [1] 0.038461538 0.001597444 1.000000000 0.137931034 0.038461538
```

We can also assign the name `frunge` to the `fRunge` function in R.

```r
julia_assign("frunge", fRunge)
julia_call("frunge", c(-1, -0.5, 0, 0.5, 1))
## [1] 0.038461538 0.137931034 1.000000000 0.137931034 0.038461538
```

We want to integrate this function applying the Gauss-Kronrod integration routine in Julia. For this the *QuadGK* package is needed.

```r
julia_library("QuadGK")
julia_command("I, err = quadgk(frunge, -1, 1);")
julia_eval("I, err")
...
The functions in *JuliaCall* are documented, as in any other R package. For example, help for the `julia_help()` command looks like
...
```
function agm(a::Number, b::Number, tol::Number)
    local a0 = a; local b0 = b
    while abs(b0-a0) >= tol
...
This function, calculated through an iteration, is not vectorized, that is the expression `agm(1, [0.5, 1.0, 1.5]` will not work out. Julia will tell us "no method matching agm(::Int64, ::Array{Float64,1})".

Instead, we can use the *dot* notation of Julia. First, define the function `ag(x) = agm(1, x)` that is still not vectorized. If `f` is a function in Julia, then for a vector `x` the expression `f.(x)` will *broacast* the function over the vector, that is apply it to each element of the vector and generate a vector of results.

```
julia_command("ag(x) = agm(1, x, 1e-15)")
## ag (generic function with 1 method)

julia_command("ag.([0.5, 1.0, 1.5])")
...
julia_command("G = 1 / agm(b1, sqrt(b2), 1e-50)")
## 8.346268416740731862814297327990468089939
##   930134903470024498273701036819927095195e-01
```
...
*JuliaCall* provides a *pipe* operator `%>J%` like so:

```r
2 %>J% sqrt         # julia> sqrt(2)
## [1] 1.414214

10 %>J% log(2)      # julia> log(10, 2)
## [1] 0.30103
```

where the first number is inserted into the first variable position of the function on the right. `log(b, x)` returns the logarithm of `x` for base `b`. Pipe operators can be concatenated.


## Plotting

Accessing the Julia plotting routines through *JuliaCall* is is a bit tricky. We suggest to employ the Julia *Plots* library, call one of the backends, *GR* or *Plotly* (*PlotlyJS* and *PyPlot* are still problematic), and with `plotsViewer()` tell Julia to open plots in a separate window.

```r
julia_library("Plots")
plotsViewer()

julia_command('gr()')       # or: plotly(), plotlyjs(), pyplot()
julia_command('x = linspace(0, 2*pi); y = sin.(x);')
julia_command('Plots.plot(x, y)')
```

When using RStudio the plot will be displayed in the 'Viewer' window (not in the 'Plots' window !), when called from the R command line, i.e. w/o RStudio, a page in the Web browser will be opened (if everything is configured rightly) containing the plot.

Please note that `julia_command('Plots.plot(x, y)')` does not have a semicolon at the end, otherwise the plot will not be displayed. We could also use `julia_call()`, then a `show_value=TRUE` will be needed in the call.

```r
x = 1:10; y = runif(10)
julia_call("Plots.plot", x, y, show_value=TRUE)

julia_assign("xx", x); julia_assign("yy", y)
julia_command("Plots.plot(xx, yy)")         # no ';' at the end
```

In the other direction, we can plot in R a function defined in Julia, such as the `ag` function above.

```r
x = seq(0, 1, length.out=1000)
julia_assign("x", x)
y = julia_eval("ag.(x)")
plot(x, y, type='l', col="blue"); grid()
```

![Function $f(x) = \mathrm{agm}(1, x)$](Rplot_agm.png)  
